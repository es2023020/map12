        ...
        <div className="flex flex-wrap items-end justify-between gap-4 mb-8">
          <div>
            ...
            <p className="mt-2 text-muted-foreground text-sm max-w-md">
              Discover the latest exclusive real estate launches across Egypt. Reserve your spot
          ...
          <div className="flex items-center gap-3">
            <Link
              to="/new-launches"
              className="inline-flex items-center gap-2 rounded-full border border-border bg-card px-4 py-2 text-xs font-semibold text-primary hover:border-accent hover:text-accent hover:bg-accent/5 transition-all"
            >
            ...
            <div className="flex gap-2">
              <Button
                ...
                size="icon"
                onClick={scrollPrev}
                className="rounded-full border-border/60 bg-card hover:bg-accent/10 hover:text-accent"
              >
          ...
          <div className="flex -ml-4 touch-pan-y">
            {sliderCompounds.map((c) => {
              const parent = c.parentSlug
                ? compoundsList.find((p) => p.slug === c.parentSlug)
                : null;
              const hasImgError = failedImages[c.slug];
              return (
                <div
                  key={c.slug}
                  className="min-w-0 shrink-0 grow-0 pl-4 basis-[85%] sm:basis-[45%] md:basis-[35%] lg:basis-[28%]"
                >
                  <Link
                    to="/projects/$slug"
                    params={{ slug: c.slug }}
                    className="group block relative rounded-2xl bg-card border border-border/40 shadow-sm transition-all hover:shadow-xl hover:-translate-y-1 overflow-hidden h-full flex flex-col"
                  >
                    <div className="relative aspect-[4/3] w-full overflow-hidden bg-secondary">
                      ...
                      <div className="absolute bottom-3 left-4 right-4">
                        <div className="text-white font-display text-lg font-bold truncate">
                          {c.name}
                        </div>
                      ...
                      <div className="mt-4 pt-4 border-t border-border/40 flex items-center justify-between">
                        <div>
                          ...
                          <div className="text-accent font-semibold">
                            {c.priceFrom > 0 ? `${c.priceFrom}M EGP` : "Price on Request"}
                          </div>
                        </div>
