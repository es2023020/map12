    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-0.5">
          Scan the{" "}
          <code className="bg-secondary/40 px-1 py-0.5 rounded text-accent font-semibold text-[10px]">
            /public/brochures
          </code>{" "}
          folder to automatically match PDF filenames with platform projects.
        </p>
      </div>
      ...
      <div className="flex gap-4">
        ...
        >
          <RefreshCw className={`h-3.5 w-3.5 ${loading ? "animate-spin" : ""}`} /> Scan brochures
          Folder
        </Button>
        {autoLinked.length > 0 && (
          <Button
            ...
            size="sm"
            variant="outline"
            className="rounded-xl text-xs border-emerald-500 text-emerald-600 hover:bg-emerald-500/10"
          >
            <Check className="h-3.5 w-3.5 mr-1" /> Auto-Link All High Confidence (
            {autoLinked.length})
          </Button>
        )}
      </div>
      ...
      {conflicts.length > 0 && (
        <div className="rounded-2xl border border-red-500/20 bg-red-500/5 p-5 space-y-3">
          ...
          <p className="text-[11px] text-red-600/80 leading-relaxed">
            These files matched projects that already have brochures uploaded or have multiple files
            matching the same project name. Manual verification required.
          </p>
          ...
          <div className="divide-y divide-red-500/10 border border-red-500/15 rounded-xl overflow-hidden bg-card text-xs">
            {conflicts.map((c, idx) => (
              <div
                key={idx}
                className="p-3.5 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-900/40"
              >
                <div>
      ...
      {autoLinked.length > 0 && (
        <div className="rounded-2xl border border-border bg-card p-5 space-y-4">
          <h3 className="text-xs font-bold text-primary uppercase tracking-wider flex items-center gap-1.5">
            <FileCheck className="h-4.5 w-4.5 text-emerald-500" /> High Confidence Matches (
            {autoLinked.length})
          </h3>
          ...
          <div className="divide-y divide-border/60 border border-border/80 rounded-xl overflow-hidden text-xs">
            {autoLinked.map((item) => {
              ...
              return (
                <div
                  key={item.filename}
                  className="p-3.5 flex items-center justify-between hover:bg-secondary/15 transition-all"
                >
                  <div className="min-w-0">
                    <span className="font-bold text-primary block truncate max-w-[280px]">
                      {item.filename}
                    </span>
                    <span className="text-[10px] text-muted-foreground block mt-0.5">
                      Fuzzy Match:{" "}
                      <span className="font-semibold text-emerald-500">{item.projectName}</span> (
      ...
      {flagged.length > 0 && (
        <div className="rounded-2xl border border-border bg-card p-5 space-y-4">
          <h3 className="text-xs font-bold text-primary uppercase tracking-wider flex items-center gap-1.5">
            <HelpCircle className="h-4.5 w-4.5 text-amber-500" /> Medium Confidence Review (
            {flagged.length})
          </h3>
          ...
          <div className="divide-y divide-border/60 border border-border/80 rounded-xl overflow-hidden text-xs">
            {flagged.map((item) => {
              ...
              return (
                <div
                  key={item.filename}
                  className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-secondary/10 transition-all"
                >
                  <div>
                    <span className="font-bold text-primary block truncate max-w-[280px]">
                      {item.filename}
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      {item.topCandidates.map((c: any) => (
                        <button
                          key={c.slug}
                          onClick={() =>
                            setManualMatches((prev) => ({ ...prev, [item.filename]: c.slug }))
                          }
                  ...
                  <div className="flex items-center gap-3 self-end sm:self-auto shrink-0">
                    <select
                      value={selectedSlug}
                      onChange={(e) =>
                        setManualMatches((prev) => ({ ...prev, [item.filename]: e.target.value }))
                      }
      ...
      {unmatched.length > 0 && (
        <div className="rounded-2xl border border-border bg-card p-5 space-y-4">
          <h3 className="text-xs font-bold text-primary uppercase tracking-wider flex items-center gap-1.5">
            <FileText className="h-4.5 w-4.5 text-slate-400" /> Unmatched PDF Files (
            {unmatched.length})
          </h3>
          <p className="text-[11px] text-muted-foreground">
            No clear project match candidates were found for these files. You can map them to any
            existing compound manually.
          </p>
