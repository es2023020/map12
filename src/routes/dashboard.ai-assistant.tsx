...
function AIAssistantPage() {
  ...
  useEffect(() => {
    ...
    leads.forEach(lead => {
      ...
      const daysSinceContact = Math.round((Date.now() - (lead.lastContacted || lead.createdAt)) / 86400000);
      ...
      const matchedCompounds = compounds.filter(c => {
        ...
        const matchesInterest = c.slug.includes(interestKeyword) || 
  ...
  const handleLaunchWhatsApp = (lead: Lead, msgText: string) => {
    ...
    window.open(`https://web.whatsapp.com/send?phone=${phoneWithCode}&text=${encodeURIComponent(msgText)}`, "_blank");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border-b border-border/40 pb-4">
        ...
        <p className="mt-1 text-sm text-muted-foreground">Built-in broker co-pilot analyzing lead pipeline priorities, generating smart reminders, and matchmaking budgets to active compounds.</p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3 items-start">
        ...
        <div className="rounded-2xl border border-border bg-card p-5 shadow-sm space-y-4">
          ...
          <div className="space-y-3">
            {reminders.map((rem) => (
              <div key={rem.id} className="rounded-xl border border-border/50 bg-secondary/15 p-3.5 space-y-2 hover:border-accent/40 transition-colors">
                <div className="flex items-center gap-2 text-xs font-bold text-accent">
                  <AlertCircle className="h-3.5 w-3.5 shrink-0" />
                  <span>Inactive Follow-up</span>
                </div>
                <p className="text-xs text-foreground/80 leading-relaxed font-medium">"{rem.text}"</p>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="rounded-2xl border border-border bg-card p-5 shadow-sm space-y-4">
            <h3 className="text-sm font-bold uppercase tracking-wider text-primary">Pipeline Matchmaking & Recommendations</h3>

            <div className="space-y-4 max-h-[550px] overflow-y-auto pr-1">
              {insights.map((insight) => (
                <div key={insight.lead.id} className="rounded-xl border border-border/60 p-4 space-y-3 bg-secondary/10">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border/30 pb-2">
                    <div>
                      ...
                      <div className="text-[10px] text-muted-foreground mt-0.5">Budget: EGP {insight.lead.budget}M • Interest: {insight.lead.interest || "General"}</div>
                    </div>

                    <div className="text-right">
                      <span className="text-[10px] text-muted-foreground">Days since contact</span>
                      <div className="text-xs font-bold text-primary mt-0.5">{insight.daysSinceContact}d</div>
                    </div>
                  </div>

                  {/* Priority Reasons */}
                  <div className="space-y-1">
                    <span className="text-[9px] font-bold text-muted-foreground uppercase">Reasoning</span>
                    <div className="flex flex-wrap gap-1.5 mt-0.5">
                      {insight.reasons.map((r: string, idx: number) => (
                        <span key={idx} className="rounded bg-background border border-border/60 px-1.5 py-0.5 text-[9px] text-muted-foreground font-semibold">
                          • {r}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="space-y-1.5 pt-1">
                    ...
                    <div className="grid gap-2 sm:grid-cols-3 mt-1">
                      {insight.matches.map((match: any) => (
                        <div key={match.slug} className="rounded-lg bg-card border border-border/60 p-2 hover:border-accent/40 transition-colors flex flex-col justify-between h-20">
                          <div className="min-w-0">
                            <div className="truncate text-[10px] font-bold text-primary">{match.name}</div>
                      ...
                      {insight.matches.length === 0 && (
                        <div className="col-span-full rounded-lg border border-dashed border-border/50 p-3 text-center text-[10px] text-muted-foreground py-6">
                          No matching compounds found within budget in "{insight.lead.interest}" area.
                        </div>
                      )}
                    </div>
                  </div>
