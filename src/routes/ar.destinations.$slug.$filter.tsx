        ...
        <div className="bg-primary text-primary-foreground py-10 px-4 sm:px-6 lg:px-8">
          <div className="max-w-7xl mx-auto">
            <Link
              ...
              className="inline-flex items-center gap-1.5 text-xs text-primary-foreground/70 hover:text-accent mb-3"
            >
              <ArrowLeft className="h-3.5 w-3.5 rotate-180" /> العودة لدليل كمبوندات {dest.name}
            </Link>
            <h1 className="font-display text-3xl sm:text-4xl font-bold tracking-tight">{seo.h1}</h1>
            <p className="mt-2 text-sm sm:text-base text-primary-foreground/80 max-w-3xl leading-relaxed">
              استعرض قائمة الوحدات والمشاريع المطابقة لـ {filter} في كمبوندات {dest.name} مع خطط
              سداد بدون فوائد واسعار استثنائية.
            </p>
          </div>
        </div>
          ...
          {filteredCompounds.length > 0 ? (
            ...
            <div className="text-center py-16 bg-card rounded-2xl border border-border">
              ...
              <h3 className="text-lg font-bold text-foreground">
                لا توجد كمبوندات مطابقة لفلتر البحث حالياً
              </h3>
              <p className="text-sm text-muted-foreground mt-1">
                تصفح جميع المشاريع المتاحة في {dest.name}.
              </p>
              <Link
                to="/ar/destinations/$slug"
                params={{ slug: dest.slug }}
                className="inline-flex items-center justify-center mt-4 px-5 py-2.5 bg-primary text-primary-foreground text-xs font-bold rounded-full"
              >
