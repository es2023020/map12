...
const stages: Array<{ id: LeadStage; label: string; color: string; border: string }> = [
  ...
  { id: "contacted", label: "Contacted", color: "bg-blue-500/10 text-blue-500", border: "border-blue-500/20" },
  { id: "viewing", label: "Viewing Site", color: "bg-amber-500/10 text-amber-500", border: "border-amber-500/20" },
  { id: "negotiating", label: "Negotiating", color: "bg-violet-500/10 text-violet-500", border: "border-violet-500/20" },
  { id: "closed", label: "Closed Won", color: "bg-emerald-500/10 text-emerald-500", border: "border-emerald-500/20" },
];
...
function LeadsPage() {
  ...
  const applyTemplate = (lead: Lead, templateType: "welcome" | "followup" | "offer", customText?: string) => {
  ...
  const handleAssignCompound = (leadId: string, compoundSlug: string, startPrice: number) => {
    ...
    if (selectedLead && selectedLead.id === leadId) {
      setSelectedLead(prev => prev ? {
        ...prev,
        assignedUnits: prev.assignedUnits.includes(unitDesc) ? prev.assignedUnits : [...prev.assignedUnits, unitDesc],
        activityLog: [
          { id: Math.random().toString(), type: "system", text: `Linked compound unit: ${unitDesc}`, timestamp: Date.now() },
          ...prev.activityLog
        ]
      } : null);
    }
  ...
  const handleRemoveCompound = (leadId: string, unitDesc: string) => {
    ...
    if (selectedLead && selectedLead.id === leadId) {
      setSelectedLead(prev => prev ? {
        ...
        activityLog: [
          { id: Math.random().toString(), type: "system", text: `Removed linked unit: ${unitDesc}`, timestamp: Date.now() },
          ...prev.activityLog
        ]
      } : null);
    }
  ...
  const handleAddManualNote = (leadId: string) => {
    ...
    if (selectedLead && selectedLead.id === leadId) {
      setSelectedLead(prev => prev ? {
        ...prev,
        activityLog: [
          { id: Math.random().toString(), type: "note", text: newNoteText, timestamp: Date.now() },
          ...prev.activityLog
        ]
      } : null);
    }
  ...
  const handleScheduleMeeting = (lead: Lead) => {
    ...
    const locationStr = meetingType === "Google Meet" ? "Google Meet Link (Auto)" : "Compound Project Site Location";
    ...
    if (selectedLead && selectedLead.id === lead.id) {
      setSelectedLead(prev => prev ? {
        ...prev,
        activityLog: [
          { id: Math.random().toString(), type: "meeting", text: logMsg, timestamp: Date.now() },
          ...(prev.activityLog || [])
        ]
      } : null);
    }
  ...
  const matchedCompounds = compounds.filter(c => 
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-border/40 pb-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Manage client cards, assign compound sheets, log call histories, and dispatch calendar slots.</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button onClick={() => setOpen(!open)} className="rounded-full bg-accent text-accent-foreground hover:bg-accent/90">
      ...
      {open && (
        <div className="rounded-2xl border border-border bg-card p-5 shadow-soft max-w-2xl animate-in fade-in duration-200">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <Input placeholder="Phone (e.g. +20100...)" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} />
            <Input type="number" placeholder="Budget (EGP M)" value={form.budget} onChange={(e) => setForm({ ...form, budget: Number(e.target.value) })} />
            <Input placeholder="Interest (compound slug)" value={form.interest} onChange={(e) => setForm({ ...form, interest: e.target.value })} />
            <Input className="md:col-span-2" placeholder="Notes (requirements, timeline...)" value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} />
          </div>
      ...
      <div className="grid gap-4 lg:grid-cols-5 items-start">
        {stages.map((stg) => {
          ...
          return (
            <div key={stg.id} className={`rounded-2xl border ${stg.border} bg-secondary/10 p-3 h-full min-h-[500px]`}>
              <div className="flex items-center justify-between px-1 border-b border-border/40 pb-2 mb-3">
                <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-bold ${stg.color}`}>{stg.label}</span>
                <span className="text-xs font-bold text-muted-foreground">{items.length}</span>
              </div>
              
              <div className="space-y-3">
                {items.map((l) => (
                  <div key={l.id} className="rounded-xl border border-border/60 bg-card p-3.5 shadow-sm hover:shadow transition-all group">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0 cursor-pointer flex-1" onClick={() => {
                        ...
                        <div className="font-semibold text-primary truncate hover:text-accent flex items-center gap-1.5">
                          {l.name}
                          {l.priority === "high" && <Flame className="h-3.5 w-3.5 text-red-500 fill-red-500 animate-pulse" />}
                        </div>
                        <div className="mt-1 flex items-center gap-1.5 text-xs text-muted-foreground">
                          <Phone className="h-3 w-3" /> {l.phone}
                        </div>
                      </div>
                      <button onClick={() => removeLead(l.id)} className="text-muted-foreground hover:text-destructive transition-colors"><Trash2 className="h-3.5 w-3.5" /></button>
                    </div>

                    <div className="mt-3 grid grid-cols-2 gap-1 border-t border-b border-border/40 py-2 my-2 text-xs">
                      <div>
                        <div className="text-[10px] text-muted-foreground uppercase font-bold">Budget</div>
                      ...
                      <div>
                        ...
                        <div className="font-semibold text-accent mt-0.5 truncate">{l.interest || "General"}</div>
                      </div>
                    </div>
                    ...
                    {(l.assignedUnits || []).length > 0 && (
                      <div className="space-y-1 my-2">
                        <span className="text-[9px] font-bold text-muted-foreground uppercase">Linked Units</span>
                        <div className="space-y-1">
                          {(l.assignedUnits || []).slice(0, 2).map((unit, idx) => (
                            <div key={idx} className="text-[10px] text-primary truncate bg-secondary/35 p-1 rounded border border-border/40 flex items-center justify-between">
                              <span className="truncate">{unit}</span>
                            </div>
                          ))}
                          {(l.assignedUnits || []).length > 2 && (
                            <div className="text-[9px] text-muted-foreground italic">+ {l.assignedUnits.length - 2} more...</div>
                          )}
                        </div>
                      </div>
                    )}

                    {l.notes && <div className="text-xs text-foreground/75 bg-secondary/20 p-2 rounded-lg line-clamp-2 leading-relaxed">{l.notes}</div>}
                ...
                {items.length === 0 && <div className="rounded-xl border border-dashed border-border/60 p-4 text-center text-xs text-muted-foreground py-10 bg-card/40">Empty stage</div>}
              </div>
            </div>
          );
        })}
          ...
          <div className="w-full max-w-xl h-full bg-card border-l border-border p-6 shadow-2xl flex flex-col justify-between overflow-y-auto animate-in slide-in-from-right duration-300">
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div>
                    <label className="text-[10px] font-bold text-muted-foreground uppercase">Sales Budget (EGP Millions)</label>
                    <input
                      type="number"
                  ...
                  <div>
                    <label className="text-[10px] font-bold text-muted-foreground uppercase">General Interest Area</label>
                    <input
                      type="text"
                      ...
                      onChange={(e) => {
                        ...
                        setSelectedLead(prev => prev ? { ...prev, interest: newInterest } : null);
                      }}
                      className="w-full mt-1.5 rounded-xl border border-border bg-transparent px-3 py-2 text-sm text-primary font-semibold focus:border-accent focus:outline-none"
                    />
                  </div>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <div>
                    <label className="text-[10px] font-bold text-muted-foreground uppercase">Lead Priority</label>
                    <select
                ...
                <div>
                  <label className="text-[10px] font-bold text-muted-foreground uppercase">Private Remarks & Call Notes</label>
                  <textarea
              ...
              <div className="border-t border-border/40 pt-5 space-y-4">
                <span className="text-xs font-bold uppercase tracking-wider text-primary flex items-center gap-1.5">
                  <LinkIcon className="h-4 w-4 text-accent" /> Assigned Project Sheets ({(selectedLead.assignedUnits || []).length})
                </span>

                <div className="space-y-2">
                  {(selectedLead.assignedUnits || []).map((unit, idx) => (
                    <div key={idx} className="flex items-center justify-between gap-3 p-2.5 rounded-xl border border-border bg-secondary/10 hover:bg-secondary/20 transition-all">
                      <div className="text-xs font-semibold text-primary truncate flex-1">{unit}</div>
                      ...
                      <div className="flex gap-1.5 shrink-0">
                        ...
                        <button onClick={() => handleRemoveCompound(selectedLead.id, unit)} className="p-1.5 rounded bg-destructive/10 text-destructive hover:bg-destructive hover:text-white transition-colors">
                  ...
                  {(selectedLead.assignedUnits || []).length === 0 && (
                    <div className="text-xs text-muted-foreground italic py-1">No units mapped. Search compounds below to assign an option.</div>
                  )}
                </div>
                    ...
                    <div className="bg-card border border-border/60 rounded-xl overflow-hidden shadow-lg mt-1 space-y-1 max-h-36 overflow-y-auto">
                      {matchedCompounds.map(comp => (
                        <button
                          key={comp.slug}
                          type="button"
                          onClick={() => handleAssignCompound(selectedLead.id, comp.slug, comp.priceFrom)}
                        ...
                        >
                          <span>{comp.name} ({comp.destination})</span>
              ...
              <div className="border-t border-border/40 pt-5 space-y-4">
                <div className="text-xs font-bold uppercase tracking-wider text-primary flex items-center justify-between gap-1.5 w-full">
                  <span className="flex items-center gap-1.5"><Calendar className="h-4 w-4 text-accent" /> Google Calendar scheduler</span>
                  {user && <span className="text-[10px] text-blue-500 font-semibold lowercase">Connected: {user.email}</span>}
                </div>
                ...
                <div className="bg-secondary/10 p-4 rounded-xl border border-border/40 space-y-3">
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div>
                      <label className="text-[9px] font-bold text-muted-foreground uppercase">Appointment Title</label>
                      <input 
                        type="text"
                        value={meetingTitle}
                    ...
                    <div>
                      <label className="text-[9px] font-bold text-muted-foreground uppercase">Date & Time</label>
                      <input 
                        type="datetime-local"
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <label className="text-[9px] font-bold text-muted-foreground uppercase">Meeting Type</label>
                        ...
                        {(["Google Meet", "Site Visit"] as const).map(t => (
                          <button
                            ...
                            className={`rounded-lg px-2.5 py-1 text-[10px] font-bold border transition-all ${
                              meetingType === t ? "border-accent bg-accent/10 text-accent" : "border-border bg-background text-muted-foreground"
                            }`}
                          >
                            {t}
                          </button>
                ...
                <div className="space-y-3 relative before:absolute before:left-3 before:top-2 before:bottom-2 before:w-[1px] before:bg-border/60 pl-6 max-h-60 overflow-y-auto pr-1">
                  {(selectedLead.activityLog || []).map((log) => (
                    <div key={log.id} className="relative space-y-1">
                      ...
                      <span className={`absolute -left-6 top-1 h-2.5 w-2.5 rounded-full border bg-background ${
                        log.type === "note" 
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="w-full max-w-md rounded-2xl border border-border bg-card p-6 shadow-xl flex flex-col animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between border-b border-border/40 pb-3 mb-4">
              ...
              <button onClick={() => setWhatsappLead(null)} className="text-muted-foreground hover:text-foreground">
                <X className="h-4 w-4" />
              </button>
            </div>
            
            <div className="space-y-4">
              <div className="text-xs text-muted-foreground">Recipient: <strong className="text-primary">{whatsappLead.name} ({whatsappLead.phone})</strong></div>
              
              {/* Quick templates chips */}
              <div className="flex gap-2">
                <button onClick={() => applyTemplate(whatsappLead, "welcome")} className="rounded bg-secondary px-2.5 py-1 text-[10px] font-bold text-primary hover:bg-secondary/80">Welcome</button>
                <button onClick={() => applyTemplate(whatsappLead, "followup")} className="rounded bg-secondary px-2.5 py-1 text-[10px] font-bold text-primary hover:bg-secondary/80">Followup</button>
                <button onClick={() => applyTemplate(whatsappLead, "offer")} className="rounded bg-secondary px-2.5 py-1 text-[10px] font-bold text-primary hover:bg-secondary/80">Proposal</button>
              </div>
              ...
              <div className="flex justify-end gap-2 pt-2">
                <Button variant="ghost" onClick={() => setWhatsappLead(null)}>Cancel</Button>
                <button 
