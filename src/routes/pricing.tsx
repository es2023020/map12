...
export const Route = createFileRoute("/pricing")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "PropTrack broker subscription plans for individual agents and brokerage teams operating in Egypt's primary property market." },
    ],
  }),
    ...
    <Shell>
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-[#0c1524] via-[#111e35] to-[#1a365d] text-white py-20">
        ...
        <div className="relative mx-auto max-w-5xl px-4 text-center lg:px-8">
          <div className="inline-flex items-center gap-2 rounded-full bg-white/5 border border-white/10 px-4 py-1.5 text-xs font-semibold backdrop-blur-md mb-5 text-indigo-300">
            <Zap className="h-3.5 w-3.5 text-yellow-400 fill-yellow-400" /> Transparent Egypt Primary Market Subscriptions
          </div>
          ...
          <p className="mt-6 text-lg text-slate-300 max-w-2xl mx-auto leading-relaxed">
            Gain an unfair analytical advantage in New Cairo, New Capital, Sheikh Zayed, and North Coast. Pay monthly with zero hidden fees.
          </p>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-7xl px-4 py-16 lg:px-8 space-y-20">
        ...
        <section>
          <div className="grid gap-8 lg:grid-cols-4 md:grid-cols-2">
            {plans.map((plan) => (
              ...
              >
                {plan.badge && (
                  <div className={`absolute -top-3 left-6 rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-wider shadow-sm ${
                ...
                <div className="mt-4">
                  <h3 className={`font-display text-xl font-bold ${plan.highlight ? "text-amber-400" : "text-slate-900 dark:text-white"}`}>
                    {plan.name}
                  </h3>
                  <div className="mt-4 flex items-baseline gap-1">
                    <span className={`font-display text-3xl font-extrabold tracking-tight ${plan.highlight ? "text-white" : "text-slate-900 dark:text-white"}`}>
                      {plan.price}
                    </span>
                ...
                <hr className={`my-5 border-dashed ${plan.highlight ? "border-slate-800" : "border-slate-200 dark:border-slate-800"}`} />

                <ul className="space-y-3 mb-8 flex-grow">
                  {plan.features.map((feature, idx) => (
                    <li key={idx} className="flex items-start gap-2.5 text-xs">
                      <Check className={`h-4 w-4 shrink-0 ${plan.highlight ? "text-amber-400" : "text-emerald-500"}`} />
                      <span className={plan.highlight ? "text-slate-300" : "text-slate-600 dark:text-slate-300"}>{feature}</span>
                    </li>
                  ))}
                </ul>
        ...
        <section className="rounded-3xl border border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-900/50 p-8 text-center shadow-sm">
          <h2 className="font-display text-xl font-bold text-slate-900 dark:text-white mb-2">💳 ADIB Bank Transfer Payments</h2>
          <p className="text-sm text-slate-600 dark:text-slate-400 max-w-xl mx-auto">
            Payments are billed monthly. Upgrade or downgrade online instantly. Mid-cycle upgrades are charged on a prorated basis; downgrades are queued to take effect on your next billing date.
          </p>
