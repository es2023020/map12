...
import { 
  Building2, Calendar, MapPin, Sparkles, Image as ImageIcon, 
  ArrowLeft, ArrowRight, Search, SlidersHorizontal 
} from "lucide-react";
...
export const Route = createFileRoute("/new-launches")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Egypt's most anticipated real estate developments, updated in real time." },
    ],
  }),
    ...
    <Shell>
      ...
      <div className="bg-gradient-to-r from-primary via-primary/95 to-accent/90 text-primary-foreground relative overflow-hidden">
        ...
        <div className="mx-auto max-w-7xl px-4 py-16 lg:px-8 relative z-10">
          <Link to="/" className="inline-flex items-center gap-1.5 text-xs font-semibold text-primary-foreground/75 hover:text-accent transition-colors mb-4">
          ...
          <p className="mt-3 text-sm md:text-base text-primary-foreground/80 max-w-xl leading-relaxed">
            A dedicated portal for premium off-plan launches, new phases, and exclusive opportunities across Ras El Hekma, New Cairo, and Sheikh Zayed.
          </p>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-7xl px-4 py-10 lg:px-8">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-border/50 pb-6 mb-8">
          ...
          <div className="text-xs text-muted-foreground font-medium">
            Showing <strong className="text-foreground">{filteredLaunches.length}</strong> of {launches.length} launches
          </div>
        </div>
              ...
              >
                <div className="relative aspect-[4/3] w-full overflow-hidden bg-secondary">
                  ...
                  <div className="absolute bottom-3 left-4 right-4">
                    <div className="text-white font-display text-base font-bold truncate">{c.name}</div>
                ...
                <div className="p-5 flex-1 flex flex-col justify-between">
                  <div>
                    ...
                    <div className="text-[10px] text-muted-foreground uppercase tracking-wider font-semibold mb-1">Developer</div>
                    <div className="text-xs font-semibold text-foreground truncate">{c.developer}</div>
                  </div>

                  <div className="mt-4 pt-4 border-t border-border/40 flex items-center justify-between">
                    <div>
                      <div className="text-[9px] text-muted-foreground uppercase tracking-wide font-medium">Starting from</div>
                      <div className="text-accent text-sm font-bold">{c.priceFrom}M EGP</div>
                    </div>
                    <div className="text-right">
                      <div className="text-[9px] text-muted-foreground uppercase tracking-wide font-medium">Delivery</div>
