...
const compounds = [...compoundsGenerated];

// Find or update La Vista City
let lvc = compounds.find((c) => c.slug === "la-vista-city");
if (lvc) {
  lvc.developer = "STM";
...
// Find or update Bamboo Extension
let be = compounds.find((c) => c.slug === "bamboo-extension");
if (be) {
  be.developer = "Palm Hills Developments";
  be.developerSlug = "palm-hills-developments";
}

// Find or update Address East
let ae = compounds.find((c) => c.slug === "address-east");
if (ae) {
...
// Add Village West
let vw = compounds.find((c) => c.slug === "village-west");
if (!vw) {
  vw = {
    slug: "village-west",
    name: "Village West",
...
// Write compounds.generated.ts
const compTsContent = `import { Compound } from "./compounds";

export const compoundsGenerated: Compound[] = ${JSON.stringify(compounds, null, 2)};
`;
fs.writeFileSync(path.join(process.cwd(), "src", "data", "compounds.generated.ts"), compTsContent, "utf-8");
console.log("[OK] Updated compounds.generated.ts");
...
// Merge into availability list
const updateAvailItem = (item: any) => {
  const idx = availList.findIndex((a) => a.slug === item.slug);
  if (idx !== -1) {
    availList[idx] = item;
  } else {
    availList.push(item);
...
updateAvailItem(lvcAvail);
updateAvailItem(aeAvail);
updateAvailItem(vwAvail);

const availTsContent = `// Auto-generated from data/availability/ — do not edit by hand.
...
`;
fs.writeFileSync(path.join(process.cwd(), "src", "data", "availability.generated.ts"), availTsContent, "utf-8");
console.log("[OK] Updated availability.generated.ts");
