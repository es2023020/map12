...
compCode = compCode.replace(
  /slug:\s*"ogami"[\s\S]*?priceFrom:\s*[\d.]+/g,
  (m) => m.replace(/priceFrom:\s*[\d.]+/, 'priceFrom: 28.0')
);
compCode = compCode.replace(
  /slug:\s*"ogami"[\s\S]*?paymentPlan:\s*"[^"]*"/g,
  (m) => m.replace(/paymentPlan:\s*"[^"]*"/, 'paymentPlan: "5% down · 7 or 8 years equal installments"')
);
compCode = compCode.replace(
  /slug:\s*"ogami"[\s\S]*?unitSizes:\s*"[^"]*"/g,
  (m) => m.replace(/unitSizes:\s*"[^"]*"/, 'unitSizes: "150–215 m²"')
);
compCode = compCode.replace(
  /slug:\s*"ogami"[\s\S]*?types:\s*\[[\s\S]*?\]/g,
  (m) => m.replace(/types:\s*\[[\s\S]*?\]/, 'types: ["Townhouse", "Twin House", "Villa"]')
);
...
function addOrReplace(slug, data) {
  if (availCode.includes(`"slug": "${slug}"`) || availCode.includes(`slug: "${slug}"`)) {
    ...
    const regex = new RegExp(`\\{\\s*(?:slug|\\"slug\\"):\\s*\\"${slug}\\"[\\s\\S]*?\\}\\s*,?\\s*\\n?`, 'g');
