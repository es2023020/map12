...
regCode = regCode.replace(/"grova-east-hills":\s*\{[\s\S]*?destination:\s*"[^"]*"/, (m) =>
  m.replace(/destination:\s*"[^"]*"/, 'destination: "6th-settlement"'),
);
regCode = regCode.replace(/"la-vista-east":\s*\{[\s\S]*?destination:\s*"[^"]*"/, (m) =>
  m.replace(/destination:\s*"[^"]*"/, 'destination: "6th-settlement"'),
);
...
console.log(
  "Updated src/data/compounds.ts with 6th Settlement destinations and brochures for Solana & Sarai",
);
