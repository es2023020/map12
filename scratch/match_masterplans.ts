...
for (const file of files) {
  ...
  if (aliases[baseNameNoExt]) {
    ...
    if (project) {
      matches.push({ file, projectSlug: project.slug, projectName: project.name, confidence: "HIGH" });
      continue;
    }
  }
  ...
  if (matchByName) {
    matches.push({ file, projectSlug: matchByName.slug, projectName: matchByName.name, confidence: "HIGH" });
    continue;
  }

  // 3. Partial match
  const partial = compoundsGenerated.find(c => normalize(c.name).includes(norm) || norm.includes(normalize(c.name)));
  if (partial) {
    matches.push({ file, projectSlug: partial.slug, projectName: partial.name, confidence: "MEDIUM" });
    continue;
  }
